...
function normalizeExpression(expr: string): string {
  ...
  let result = expr.replace(/[a-zA-Z_]\w*/g, (match) => {
    ...
    if (KNOWN_NAMES.has(match)) return match;
  ...
  result = result.replace(/([a-zA-Z_]\w*)\s*\(/g, (match, name) => {
    if (KNOWN_NAMES.has(name)) return match;
    return `${name} * (`;
  });
 ...
 * (`pi`, `e`) are excluded.
 */
function freeVars(...nodes: MathNode[]): Set<string> {
  ...
  for (const node of nodes) {
    node.traverse((n: MathNode) => {
      if (n.type === 'FunctionNode') {
        fnNames.add((n as any).fn.name as string);
      }
      if (n.type === 'SymbolNode') {
        allSyms.add((n as any).name as string);
      }
    });
  }
  ...
  for (let i = 0; i < TEST_VALUES.length; i++) {
    ...
    try {
      ...
      if (typeof valA !== 'number' || typeof valB !== 'number') {
        skipCount++;
        if (skipCount > maxSkips) return false;
        continue;
      }
...
function checkEquivalence(
  ...
  tolerance: number,
  definitions?: Record<string, string>,
): { nodeA: MathNode; nodeB: MathNode } | null {
  let nodeA: MathNode;
  let nodeB: MathNode;
 ...
 */
function unwrapUnaryMinus(node: MathNode): {
  inner: MathNode;
  negative: boolean;
} {
  if (
    ...
    return { inner: (node as any).args[0], negative: true };
  }
 ...
 */
function integerValue(node: MathNode): number | null {
 ...
 */
function reducibleFractionPenalty(node: MathNode): number {
  let penalty = 0;
  ...
  node.traverse((n: MathNode) => {
    if (
      n.type !== 'OperatorNode' ||
      (n as any).op !== '/' ||
      (n as any).args?.length !== 2
    ) {
      return;
    }

    const [numNode, denNode] = (n as any).args as MathNode[];
 ...
 */
export function mValueCheck(
  answer: string,
  input: string,
  options?: MValueCheckOptions,
): boolean {
 ...
 */
export function mExprCheck(
  expected: string,
  input: string,
  options?: MExprCheckOptions,
): boolean {
  const tolerance = options?.tolerance ?? DEFAULT_TOLERANCE;
